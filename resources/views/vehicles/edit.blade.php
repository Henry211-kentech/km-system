...
<div class="bg-white rounded-lg shadow p-6 max-w-2xl">
    <form action="{{ route('vehicles.update', $vehicle) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="car_model" value="{{ $vehicle->car_model }}" class="w-full px-4 py-2 border rounded @error('car_model') border-red-500 @enderror" required>
            @error('car_model')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="text" name="number_plate" value="{{ $vehicle->number_plate }}" class="w-full px-4 py-2 border rounded @error('number_plate') border-red-500 @enderror" required>
            @error('number_plate')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
